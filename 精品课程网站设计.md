...
| school_id    | `varcahr(32) not null`    | 学校id，与学校绑定             |
| major_id     | `varchar(32) not null`    | 专业id，与专业绑定             |
...
| user_id      | `varcahr(32) not null`    | 用户id，与用户账号绑定         |
| school_id    | `varchar(32) not null`    | 学校id，与学校绑定             |
...
| course_updated | `datatime() not nulldefault now()`   | 课程更新日期                          |
...
| section_name   | `varcahr(16) not null`           | 章节名称，最大长度16字符，不能为空  |
...
| resource_path | `varchar(64) not null`    | 资源路径地址，上传后自动获取                    |
...
| ----------- | ----------------------------- | ---------------------------------------------- |
| inner_id    | `varchar(32) primary key`     | 中间表id，主键，无实际意义，uuid               |
| class_id    | `varhcar(32) not null`        | 班级id，绑定班级                               |
...
| class_id    | `varchar(32)`             | 班级id，与班级绑定               |
| user_id     | `varchar(32) `            | 用户id，与用户绑定               |
| section_id  | `varcahr(32)`             | 章节id，与章节绑定               |
...
| 字段       | 字段类型                      | 字段描述                           |
| ---------- | ----------------------------- | ---------------------------------- |
| label_id   | `varcahr(32) primary key`     | 标签id，主键，无意义，uuid         |
| label_name | `varcahr(16) not null unique` | 标签名称，最大长度16字符，不能重复 |
...
| 字段      | 字段类型                  | 字段描述                         |
| --------- | ------------------------- | -------------------------------- |
| inner_id  | `varcahr(32) primary key` | 中间表id，主键，无实际意义，uuid |
| label_id  | `varchar(32) not null`    | 标签id，与标签绑定               |
| course_id | `varcahr(32) not null`    | 课程id，与课程绑定               |
...
| course_id       | `varcahr(32) not null`           | 课程id，与课程绑定                    |
| user_id         | `varchar(32) not null`           | 用户id，与用户绑定                    |
| parent_id       | `varcahr(32) not null default 0` | 父评论id，如为根评论默认为0           |
| comment_content | `varcahr(128) not null`          | 评论内容，最大长度128个字符，不能为空 |
...
| created  | `datatime not null default now()` | 创建时间                         |
